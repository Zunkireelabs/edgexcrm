...
**Three pitfalls that keep biting:**
...
- **`count: "exact"` + a `!inner` resource-embed over an RPC-based query (not a plain `.from()` table) 503s with a Postgres `42703` on the embedded column, even after the grant above is in place.** PostgREST composes the exact count by wrapping the RPC call as `pgrst_call` and adding a `count(*) OVER()` window function alongside the row projection — combined with the embed's `referencedTable`-scoped filter, that composition resolves the filter against `pgrst_call` (the RPC's own return row, typed as `leads` — e.g. `owner_id` is a real column there, `lead_collaborators.user_id` is not) instead of the joined table. Confirmed live: a branch manager applying the leads "Collaborators" filter on first load (`count=1`, `src/app/(main)/api/v1/leads/route.ts`) got `42703 "column pgrst_call.user_id does not exist"` (2026-08-25). The plain-table case is unaffected and already proven safe (`badge-counts/route.ts`'s own `!inner` + `count:exact`) — only the RPC-based case breaks. **Fix pattern (no migration needed): split the count into its own `head: true` call built from the exact same filter chain as the data query** (`route.ts`'s `buildScopedQuery(execOpts)` closure), so the two can never drift — see the `§COUNT-SPLIT` comment in that file for the full mechanism, and `route.test.ts`'s two matching regression tests (one proving the split fires, one proving it's *not* invoked prematurely on a `?facets=`-only request, which never reads the count at all).
...
- **Stage = clone of prod** (point-in-time 2026-06-21): identical schema + all rows, with **every auth password reset to `edgexdev123`**. Log into dev/local as any prod email (e.g. `sadin@zunkireelabs.com`, `hello@admizz.org`) with password `edgexdev123`.
