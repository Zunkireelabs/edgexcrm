-- Migration 208: lead_aggregates() — add a 'destination' dimension
--
-- The "Destinations" filter field (registry/leads.ts, custom_fields.countries /
-- leads.destinations dual-read) has shipped in the Advanced Filters "Add filter"
-- picker with NO option source at all — unlike every other pickable field, nothing
-- in leads-table.tsx's optionOverrides supplies it a value list, and it has no
-- static FieldDef.options either. Picking "Destinations" -> "is any of" renders a
-- permanently empty checklist: there is no path to ever select a value. Found
-- during the same production-readiness audit that found the Collaborators gap
-- (migration 207) — same root cause class (a picker with no data behind it), but
-- total rather than partial: the field has never worked.
--
-- leads.destinations is a plain TEXT[] column (migration 059, backfilled from
-- custom_fields by 087/172), so this is the same shape of fix as 207: one more
-- UNION ALL branch off lead_aggregates()'s existing `v` CTE, unnesting the array
-- and grouping by value. CREATE OR REPLACE, same signature as 194/207 — no new
-- params.
--
-- Additive only. Wrap in BEGIN/COMMIT.
--   Expected before/after row counts: 0 rows touched (function DDL only).
--   Rollback: re-apply migration 207 (CREATE OR REPLACE back to its body).
...
AS $$
  ...
  -- ── NEW: destination. leads.destinations is TEXT[] NOT NULL DEFAULT '{}' (mig
  -- 059) — no uniqueness guarantee within one lead's array like lead_collaborators
  -- has, so count(DISTINCT v.id) (not count(*)) is the exact per-destination lead
  -- count even if a lead somehow has the same country twice in its array. A lead
  -- with an empty array contributes nothing (no "(none)" sentinel for this axis,
  -- same shape as collaborator's inner-join-only above).
  UNION ALL
  SELECT 'destination', trim(dest), 'all', count(DISTINCT v.id)
  FROM v, unnest(v.destinations) AS dest
  WHERE trim(dest) <> ''
  GROUP BY 1,2,3
...
INSERT INTO public.schema_migrations (version) VALUES ('208_lead_aggregates_destination_facet.sql')
