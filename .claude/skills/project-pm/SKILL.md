...
| `/coo-real-estate` | Operating strategy (real_estate) | CRE-capital / investor-relations brain for `real_estate` (CRE sponsor / capital-raise) tenants — whole-firm product direction, end-to-end raise+IR workflow design/critique (source→structure→raise→subscribe→close→distribute→report→re-up), AI-native touchpoint hunting; knows SponsorCloud/Juniper Square/InvestNext/Covercy and the CRE Capital Management prospect. Sits ABOVE crm-expert/hr-expert and orchestrates them; advises & routes. (One COO per industry.) |
