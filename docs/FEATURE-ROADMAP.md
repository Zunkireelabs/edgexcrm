...
Last updated: 2026-07-09 (it_agency Delivery cockpit Phase 1+1.5 SHIPPED TO STAGE; delivery gap backlog from pm/coo analysis logged; Tier-0/Tier-1 + AI-synth vision-UI briefs in progress. Prior: 2026-06-20 Lead Lists.)
...
- **Tier 2 — See the money.** (a) **Cost rate** on people/time → **gross-margin** per project/client (system holds only *billing* rates today; margin is uncomputable). M. (b) **Milestone-triggered invoicing spine** — no invoices table anywhere; milestone acceptance is the natural billing trigger firing into a void; the account "Billing" tab is a stub. L. Depends on Tier-1 billing contact + amounts. *(Write an Opus-review brief before building the invoicing spine — schema-heavy.)*
- **Tier 3 — Structure + retention.** (a) **Structured status-report sections** (Accomplishments / Next / Risks / Asks + period-diff) — the scaffold the AI-synth draft fills; do *before* AI. S–M. (b) **Flip `is_client_visible` + client status share** — the flag and the proposal public-share-token pattern already exist. S.
- **Tier 4 — Methodology depth.** Sprints/cycles + burndown/velocity (L), Risk register / RAID "R" (M), milestone↔task phases (M), allocation date-bounds + over-allocation conflict (M), engagement-typed project templates (M), unified approvals inbox (timesheets + milestones + CRs) (M), timesheet submission + "who hasn't logged" compliance (M), portfolio / cross-project health roll-up (M), task dependencies / blocked-by (M–L), structured retro + project close/archive (S–M), delivery notifications/reminders (M), full client-facing read-only portal (L).
