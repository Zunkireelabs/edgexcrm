...
- **BUG (quality, not correctness): the Lead Triage score rubric is mis-calibrated** (universal) — **observed on live stage output 2026-07-27.** A lead the agent itself flagged as *"identical to an existing lead … possible duplicate"* scored **100/100**; another scored 90/100 while the reasoning noted it had no email and no phone. The prose is sensible and the number contradicts it, because `leadTriageAgent`'s `systemPrompt` (`src/lib/ai/agents/registry.ts`) asks for "a 0-100 fit/quality score" with **no rubric** for what high vs low means. Prompt-level fix: state what a duplicate, a missing contact method, and a strong lead should each score. Size S. Worth pairing with the scoring-path decision above.
