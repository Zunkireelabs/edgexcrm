...
- **Campaigns (prediction leaderboard)** — industry-scoped `education_consultancy` (Admizz); **Phase 1 + 1.5 + 1.6 SHIPPED TO PROD ✅ 2026-06-15; Phase 2 remaining.** Call sign **`CAMPAIGN-KICKOFF`**. Brief: **`docs/CAMPAIGNS-BRIEF.md`**. Live: "Campaigns" nav + FIFA WC 2026 "Predict & Win" leaderboard on `worldcup-predict-win` (ESPN auto-fetch/score/rank, admin-only) + **public masked API** for the external landing page + gear "Agent prompt" LLM handoff. Data-driven so campaign #2 + other industries plug in later. **Phase 2 (remaining):** admin manual result-override UI + integrity flags (same-phone/name clusters).
