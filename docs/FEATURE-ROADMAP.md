...
- **it_agency Delivery — Tier 1 Deal/Proposal → Project handoff** (industry-scoped `it_agency`) — **✓ BUILT ON BRANCH 2026-07-10 (mig 129 → renumbers to 134 at merge; pending merge).** `convert-to-project` drops the accepted proposal's line-item hours / total / rate / scope narrative + the deal's billing contact, forcing the PM to re-key the baseline from memory at the Qualify gate ("estimate amnesia"). Seed the project brief / baseline-estimate / budget / rate and copy contacts from the won deal + proposal; stamp baseline provenance into `project_events`. Likely needs a proposal→project link migration. Size M.
