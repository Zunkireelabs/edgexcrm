...
- **✅ DECIDED 2026-07-28 (Sadin) — keep both paths, with an explicit reason. No build.** `/api/v1/leads/[id]/insights` remains the **only** writer of `leads.ai_score`; `propose_score` stays **advisory** in the review queue, where its reasoning renders next to the number. **The reasoning is the valuable output, the number is the unreliable one** — the duplicate-vs-not judgement is solid (verified in #303) while the middle bands are noise in both directions (see the sub-bullet below), so wiring an agent score into a column users filter and sort on would degrade a working surface with no provenance to say which path wrote it. Re-open if/when band precision is computed in code rather than prompted. The original open-question text is retained below for the diagnosis.
- **DECISION NEEDED (SUPERSEDED — see the resolution directly above): two AI scoring paths exist, with opposite governance** (universal) — **found 2026-07-27 while scoping Phase-6 slice 6.1.** `leads.ai_score` / `ai_priority` / `ai_score_updated_at` are a **live, surfaced feature**: `/api/v1/leads/[id]/insights` computes a score and **writes it straight onto the lead with no approval gate at all**, surfaced in the lead-detail **AI Insights tab** and as an `ai_score` column in the leads table. Separately, Lead Triage's `propose_score` produces a score suggestion that **nothing can act on** — accepting it only flips `agent_outputs.status`. So the same product has one ungoverned AI write and one governed-but-inert AI proposal, neither aware of the other.
...
- **BUG: Lead Triage intermittently invents a task assignee** (universal) — **observed on live stage 2026-07-27.** `create_task`'s input schema says verbatim *"Omit unless the user explicitly names someone else — never invent an assignee"*; the agent supplied `ffffffff-ffff-ffff-ffff-ffffffffffff` anyway on one proposal, while other proposals correctly resolved to "You". **Caught and refused** — `createTaskCore` validates tenant membership, so the approved write failed closed with `Validation failed: {"assignee_id":["Not a member of this tenant"]}`, no task created, `ai_write_actions` recorded `failed`. **No data risk; the governance worked exactly as designed.** The cost is a wasted human approval cycle and an opaque failure at the point of approval, which trains users to distrust the queue. **UPDATE 2026-07-27, post-6.2: the prompt fix was tried and FAILED — the rate got worse, not better.** 6.2 added *"Never pass an assigneeId — omit it every time"* to `leadTriageAgent`'s prompt. Measured on stage across every `write_action_proposal` ever produced: **before the fix 1 of 3 invented an assignee; after it, 2 of 2** (`ffffffff-ffff-ffff-ffff-ffffffffffff` at 12:13, then a plausible-looking fabricated `00cfb61c-…` at 12:32 — deploy completed 12:09:26, so both runs had the new prompt). Likely mechanism: naming the field prominently raises its salience, and negated instructions are weak ("don't think of an elephant"). **Do not attempt another prompt iteration.**
  ...
  - **✅ FIXED in #305 (6.4), on prod 2026-07-28 via #306.** `agentSuppressedInputFields: ["assigneeId"]` on `createTaskTool`; `proposeAgentWrite` strips it before the `agent_outputs` insert. Both agent call sites wired (background-agent wrapper + MCP route); chat path untouched. **Verified end-to-end on stage:** the post-fix run at 14:24 proposed with no `assigneeId` key at all, and the resulting approved task landed on `assignee_id = d23c24e2…` = **the approver** (`admin@zunkireelabs.com`), `assigned_by_id` NULL — exactly `create-task.ts:138`'s `body.assignee_id ? … : defaultAssigneeId` branch.

- **KNOWN LIMITATION (logged 2026-07-28, deliberately not built): the suppressed-field strip is enforced at the persistence boundary only, not at execution.** `agentSuppressedInputFields` appears in `write-executor.ts` and `mcp/route.ts` — **never in the approval-execution path**, which re-trusts whatever `agent_approvals.tool_input` already holds. So a payload persisted *before* 6.4, or by any future path that bypasses `proposeAgentWrite`, still carries the suppressed field into execution. **Live proof on stage:** approval `2ce35d23` (pre-fix, 12:32) still holds `assigneeId: 00cfb61c-…`, a user who is **not a member of the tenant** — approving it would not assign to the approver, it would fail closed on `create-task.ts:141`'s "Not a member of this tenant". **Exposure today is nil** (prod has 0 agent identities → 0 runs → 0 stored proposals) and every *new* proposal is stripped at write time, so this was logged rather than built. **Revisit before the first tenant hires an agent on prod.** Fix shape if taken: apply the same per-tool declaration in the approval-execution path so both boundaries enforce it. Size S.

- **KNOWN NO-OP (logged 2026-07-28, deliberately left in place): `registry.ts`'s "Never pass an assigneeId — omit it every time" instruction is dead weight.** It is 6.2's failed prompt fix, measured ineffective (2 of 2 runs invented an id *with* it in place), and 6.4's structural strip now makes model behavior on this field irrelevant. Left in deliberately: removing it is a **prompt change**, and per this track's own through-line every prompt change needs its own before/after measurement to be trustworthy — that is a real measurement cycle spent on a line that changes no behavior. **The hazard is documentation, not correctness:** the next person reading the prompt may conclude `assigneeId` suppression is prompt-driven. It is not — it is `write-executor.ts`'s `stripAgentSuppressedFields`. Size XS, only worth doing if bundled with another prompt slice that is already being measured.
