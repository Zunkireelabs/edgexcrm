...
interface BranchSwitcherProps {
  ...
  /** Stretch to fill the container (used in the sidebar). */
  fullWidth?: boolean;
}
...
export function BranchSwitcher({
  ...
  if (leadScope !== "all" && userBranchId) {
    ...
    return (
      <div className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-blue-50 border border-blue-200 text-xs font-medium text-blue-700 select-none ${fullWidth ? "w-full" : ""}`}>
    ...
    return (
      <div className={`relative flex items-center ${fullWidth ? "w-full" : ""}`}>
        ...
        <select
          ...
          className={`pl-7 pr-6 py-1.5 text-xs font-medium rounded-lg border border-gray-300 bg-white text-gray-700 hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-primary/20 appearance-none cursor-pointer ${fullWidth ? "w-full" : ""}`}
        >
