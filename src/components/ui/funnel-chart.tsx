...
export interface FunnelStage {
  ...
  // percentage badge always reflect the real number. Use this when raw values are too skewed
  // for a linear shape to stay legible (e.g. one dominant stage vs. many tiny ones).
  visualValue?: number;
  displayValue?: string;
  color?: string;
...
function SegmentLabel({
  ...
  const pctEl = showPercentage && (
    <span className="whitespace-nowrap rounded-full bg-foreground px-3 py-1 font-bold text-background text-xs shadow-sm">
      {formatPercentage(pct)}
    </span>
  );
  const labelEl = showLabels && (
    <span
      className="block w-full truncate text-center font-medium text-muted-foreground text-xs leading-tight"
      title={stage.label}
    >
      {stage.label}
    </span>
