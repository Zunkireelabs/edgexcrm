    ...
    <Popover>
      <PopoverTrigger asChild>
        <button
          ...
          className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-md border border-input bg-background px-2.5 text-xs font-medium text-foreground outline-none"
        >
      ...
      <PopoverContent align="start" className="w-48 p-0">
        <Command>
          <CommandList>
            <CommandGroup>
              {operators.map((op) => (
                <CommandItem key={op} value={OPERATOR_LABELS[op]} onSelect={() => onChange(op)}>
                  <Check className={cn("size-3.5", value === op ? "opacity-100" : "opacity-0")} />
