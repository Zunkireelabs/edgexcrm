...
// Renders POST /blasts/[id]/audience-preview (SMS-PHASE4-PHASE3-BRIEF.md
// Piece B) — a paginated table of the exact leads a filter matches, opened
// on demand from the "Preview recipients" link next to the persistent
// audience-count line. Modeled on cost-preview-dialog.tsx's fetch-on-open
// pattern. MVP columns only: Name, Phone, Source — Stage/Status needs
// list-name-resolution joins resolveAudience() doesn't do yet.
...
import type { SmsAudiencePreviewResponse } from "../lib/types";
...
export function RecipientsPreviewDialog({ open, onOpenChange, blastId, audienceFilter }: RecipientsPreviewDialogProps) {
  ...
  const [preview, setPreview] = useState<SmsAudiencePreviewResponse | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Reset to page 1 whenever the dialog opens or the underlying filter changes.
  useEffect(() => {
    if (!open) return;
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    smsSend<SmsAudiencePreviewResponse>(`/api/v1/sms/blasts/${blastId}/audience-preview`, "POST", {
