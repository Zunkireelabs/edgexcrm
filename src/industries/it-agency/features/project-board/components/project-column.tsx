...
export function ProjectColumn({ status, projects, teamMap }: ProjectColumnProps) {
  const cfg = COLUMN_CONFIG[status];

  return (
    <div className={`flex flex-col gap-2 min-w-[220px] w-[220px] ${cfg.muted ? "opacity-60" : ""}`}>
      ...
      <div className="flex flex-col gap-2">
        {projects.length === 0 ? (
          ...
          projects.map((p) => <ProjectCard key={p.id} project={p} teamMap={teamMap} />)
        )}
      </div>
    </div>
  );
