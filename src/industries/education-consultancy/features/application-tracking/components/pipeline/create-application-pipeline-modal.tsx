...
interface CreateApplicationPipelineModalProps {
  open: boolean;
...
export function CreateApplicationPipelineModal({ open, onClose, pipelines }: CreateApplicationPipelineModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: Record<string, unknown> = { name: name.trim(), template };
      if (template === "copy" && copyFromId) payload.copy_from_id = copyFromId;
      ...
      if (!res.ok) throw new Error(json.error?.message || "Failed to create pipeline");
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Create Application Pipeline</DialogTitle>
        </DialogHeader>

        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          <div className="space-y-2">
            <Label htmlFor="app-pipeline-name">Pipeline Name</Label>
            <Input
              id="app-pipeline-name"
              ...
              <label className="flex items-start gap-3 p-3 rounded-lg border cursor-pointer hover:bg-muted/50 transition-colors">
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    Shortlisted, Documents Pending, Applied, Conditional Offer, Unconditional Offer, Offer Accepted, Visa Applied, Visa Approved, Enrolled, Rejected
                  </div>
                </div>
              </label>
