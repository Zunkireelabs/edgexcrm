...
import type { ApplicationStageWithCount } from "@/types/database";

interface ApplicationStageEditorProps {
  open: boolean;
  onClose: () => void;
  onSave: (data: {
    name: string;
...
export function ApplicationStageEditor({ open, onClose, onSave, stage, mode }: ApplicationStageEditorProps) {
  const [name, setName] = useState(stage?.name || "");
  const [color, setColor] = useState(stage?.color || "#3b82f6");
  const [stageType, setStageType] = useState<"regular" | "won" | "lost">(
    stage?.terminal_type === "won" ? "won" : stage?.terminal_type === "lost" ? "lost" : "regular"
  );
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="sm:max-w-[400px]">
        <DialogHeader>
          <DialogTitle>{mode === "add" ? "Add Stage" : "Edit Stage"}</DialogTitle>
        </DialogHeader>

        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          <div className="space-y-2">
            <Label htmlFor="app-stage-name">Stage Name</Label>
            <Input
              id="app-stage-name"
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Terminal stages mark applications as won or lost.
            </p>
          </div>
