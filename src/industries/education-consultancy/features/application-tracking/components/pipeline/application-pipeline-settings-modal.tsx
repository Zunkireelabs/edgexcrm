...
import { ApplicationStageEditor } from "./application-stage-editor";
import type { ApplicationPipelineWithCounts, ApplicationStageWithCount } from "@/types/database";

interface ApplicationPipelineSettingsModalProps {
  open: boolean;
  onClose: () => void;
  pipeline: ApplicationPipelineWithCounts;
}
...
function SortableStageItem({
  ...
  stage: ApplicationStageWithCount;
  onEdit: (s: ApplicationStageWithCount) => void;
  onDelete: (s: ApplicationStageWithCount) => void;
}) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="flex items-center gap-2 p-2 bg-background border rounded-lg group">
      ...
      <div className="flex items-center gap-1.5">
        ...
        {stage.application_count > 0 && (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground">{stage.application_count} apps</span>
        )}
      </div>
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <Button
          ...
          disabled={stage.application_count > 0}
          title={stage.application_count > 0 ? "Cannot delete stage with applications" : "Delete stage"}
        >
...
export function ApplicationPipelineSettingsModal({ open, onClose, pipeline }: ApplicationPipelineSettingsModalProps) {
  const [name, setName] = useState(pipeline.name);
  const [isDefault, setIsDefault] = useState(pipeline.is_default);
  const [stages, setStages] = useState<ApplicationStageWithCount[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  const [editingStage, setEditingStage] = useState<ApplicationStageWithCount | null>(null);
  const [isAddingStage, setIsAddingStage] = useState(false);
  const [deleteConfirm, setDeleteConfirm] = useState<ApplicationStageWithCount | null>(null);
  ...
  const fetchDetails = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`/api/v1/application-pipelines/${pipeline.id}`);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    setStages(newStages);
    try {
      const res = await fetch(`/api/v1/application-pipelines/${pipeline.id}/stages/reorder`, {
        method: "POST",
  ...
  const handleDeletePipeline = async () => {
    ...
    if (pipeline.application_count > 0) { toast.error(`Cannot delete pipeline with ${pipeline.application_count} applications`); return; }
    try {
      const res = await fetch(`/api/v1/application-pipelines/${pipeline.id}`, { method: "DELETE" });
      if (!res.ok) {
        const json = await res.json();
  ...
  const handleStageEditSave = async (stageData: {
    ...
    try {
      const res = await fetch(`/api/v1/application-pipelines/${pipeline.id}/stages/${editingStage.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleAddStage = async (stageData: {
    ...
    try {
      const res = await fetch(`/api/v1/application-pipelines/${pipeline.id}/stages`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(stageData),
      });
  ...
  const handleDeleteStage = async (stage: ApplicationStageWithCount) => {
    try {
      const res = await fetch(`/api/v1/application-pipelines/${pipeline.id}/stages/${stage.id}`, { method: "DELETE" });
      if (!res.ok) {
        const json = await res.json();
        ...
        <DialogContent className="sm:max-w-[500px] max-h-[85vh] overflow-hidden flex flex-col">
            ...
            {isLoading ? (
              ...
              <>
                <div className="space-y-2">
                  <Label htmlFor="app-pipeline-settings-name">Pipeline Name</Label>
                  <Input id="app-pipeline-settings-name" value={name} onChange={(e) => setName(e.target.value)} />
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <Label htmlFor="app-is-default" className="font-normal cursor-pointer">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2 border-t pt-4">
            <Button
              ...
              disabled={pipeline.is_default || pipeline.application_count > 0}
