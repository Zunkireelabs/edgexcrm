...
async function fetchUnsubscribeData(token: string): Promise<UnsubscribeData> {
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
  try {
    const res = await fetch(`${baseUrl}/api/public/email/unsubscribe/${token}`, {
      cache: "no-store",
    });
    if (!res.ok) return { valid: false, reason: "This link is no longer valid." };
    const json = await res.json();
    return json.data as UnsubscribeData;
  } catch {
    return { valid: false, reason: "This link is no longer valid." };
  }
}

export default async function EmailUnsubscribePage({ params }: PageProps) {
  const { token } = await params;
  const data = await fetchUnsubscribeData(token);
  ...
  return <UnsubscribeForm token={token} tenantName={data.tenantName!} maskedEmail={data.maskedEmail!} />;
}
