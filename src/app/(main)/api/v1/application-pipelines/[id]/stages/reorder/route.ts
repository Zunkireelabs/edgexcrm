...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const log = createRequestLogger({ requestId, method: "POST", path: `/api/v1/application-pipelines/${pipelineId}/stages/reorder` });
  ...
  const updates = (stageIds as string[]).map((stageId, index) =>
    db.from("application_stages").update({ position: index }).eq("id", stageId)
  );
  ...
  log.info({ pipelineId, stageCount: stageIds.length }, "Application stages reordered");
