...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  const log = createRequestLogger({ requestId, method: "PATCH", path: `/api/v1/application-pipelines/${pipelineId}/stages/${stageId}` });
  ...
  if (error) {
    log.error({ error }, "Failed to update application stage");
    return apiError("DB_ERROR", "Failed to update stage", 500);
  }

  log.info({ stageId }, "Application stage updated");
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
  const { id: pipelineId, stageId } = await context.params;
  const requestId = crypto.randomUUID();
  const log = createRequestLogger({ requestId, method: "DELETE", path: `/api/v1/application-pipelines/${pipelineId}/stages/${stageId}` });
  ...
  if (appCount && appCount > 0) {
    return apiConflict(`Cannot delete stage with ${appCount} applications. Move the applications first.`);
  }
  ...
  if (error) {
    log.error({ error }, "Failed to delete application stage");
  ...
  log.info({ stageId }, "Application stage deleted");
