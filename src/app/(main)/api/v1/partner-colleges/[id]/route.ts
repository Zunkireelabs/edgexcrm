...
export async function PATCH(request: NextRequest, { params }: Props) {
  ...
  const log = createRequestLogger({ requestId, method: "PATCH", path: `/api/v1/partner-colleges/${id}` });
  ...
  const { data: existing } = await db.from("partner_colleges").select("id").eq("id", id).maybeSingle();
  if (!existing) return apiNotFound("Partner college");
  ...
  if (error) {
    ...
    log.error({ error }, "Failed to update partner college");
    return apiError("DB_ERROR", "Failed to update partner college", 500);
  }

  log.info({ collegeId: id }, "Partner college updated");
...
export async function DELETE(_request: NextRequest, { params }: Props) {
  const { id } = await params;
  const requestId = crypto.randomUUID();
  const log = createRequestLogger({ requestId, method: "DELETE", path: `/api/v1/partner-colleges/${id}` });
  ...
  const { error } = await db.from("partner_colleges").delete().eq("id", id);
  if (error) {
    log.error({ error }, "Failed to delete partner college");
    return apiError("DB_ERROR", "Failed to delete partner college", 500);
  }

  log.info({ collegeId: id }, "Partner college deleted");
