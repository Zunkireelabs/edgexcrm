...
// GET /api/v1/email-blasts — paginated list.
export async function GET(request: NextRequest) {
  const requestId = crypto.randomUUID();
  const log = createRequestLogger({ requestId, method: "GET", path: "/api/v1/email-blasts" });
  ...
  if (error) {
    log.error({ err: error }, "Failed to list email blasts");
    return apiServiceUnavailable("Failed to list email blasts");
  }
...
// POST /api/v1/email-blasts — create a draft. A draft is inert until /send,
// but only reps trusted to send should be able to stage one — same posture
// as sms/blasts POST.
export async function POST(request: NextRequest) {
  const requestId = crypto.randomUUID();
  const log = createRequestLogger({ requestId, method: "POST", path: "/api/v1/email-blasts" });
  ...
  const { valid, errors } = validate(body as Record<string, unknown>, {
    ...
    subject_template: [required("subject_template"), optionalMaxLength(200)],
    body_template: [required("body_template")],
  });
  ...
  const { data, error } = await db
    ...
    .insert({
      ...
      subject_template: (body as Record<string, unknown>).subject_template,
      body_template: (body as Record<string, unknown>).body_template,
      from_name_override: (body as Record<string, unknown>).from_name_override ?? null,
  ...
  if (error) {
    log.error({ err: error }, "Failed to create email blast");
    return apiServiceUnavailable("Failed to create email blast");
  }
